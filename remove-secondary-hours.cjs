...
// Remove regularSecondaryOpeningHours from all restaurants
restaurants.forEach((restaurant, index) => {
  if (restaurant.google_data?.regularSecondaryOpeningHours !== undefined) {
    delete restaurant.google_data.regularSecondaryOpeningHours;
    removedCount++;
  }
});

console.log(`Removed regularSecondaryOpeningHours from ${removedCount} restaurants\n`);
